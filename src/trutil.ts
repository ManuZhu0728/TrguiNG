...
export function secondsToHumanReadableStr(value: number): string {
  ...
  let s = "";
  if (duration.days > 0) s = `${duration.days}${i18n.t("units.days")}`;
  if (duration.hours > 0)
    s = (s !== "" ? s + " " : "") + `${duration.hours}${i18n.t("units.hours")}`;
  if (duration.minutes > 0)
    s =
      (s !== "" ? s + " " : "") +
      `${duration.minutes}${i18n.t("units.minutes")}`;
  if (duration.seconds > 0)
    s =
      (s !== "" ? s + " " : "") +
      `${duration.seconds}${i18n.t("units.seconds")}`;
  return s;
}
...
export function timestampToDateString(value: number, config: Config): string {
  ...
  if (config.values.interface.useCustomDateTimeFormat) {
    ...
    const timeFormat =
      config.values.interface.timeFormat === "12h" ? "hh:mm:ss a" : "HH:mm:ss";
...
export function reorderElements<T>(
  list: T[],
  startIndex: number,
  endIndex: number
): T[] {
...
export function pathMapFromServer(path: string, config: ServerConfig) {
  ...
  for (const mapping of config.pathMappings) {
    if (
      ...
      normalizedPath.startsWith(normalizePath(mapping.from))
    ) {
...
export function pathMapToServer(path: string, config: ServerConfig) {
  ...
  for (const mapping of config.pathMappings) {
    if (
      mapping.to.length > 0 &&
      normalizedPath.startsWith(normalizePath(mapping.to))
    ) {
...
export function eventHasModKey(
  event: React.MouseEvent<Element> | React.KeyboardEvent<Element>
) {
  return (
    (navigator.platform.startsWith("Mac") && event.metaKey) ||
...
export function mergeTrackerLists(
  ...
  newTrackers.forEach((tier, i) => {
    if (i >= mergedTrackers.length) {
      const filteredTier = tier.filter(
        (tracker) => !uniqueTrackers.has(tracker)
      );
...
export function torrentProgressbarStyle(torrent: Torrent, config: Config) {
  ...
  if (config.values.interface.colorfulProgressbars) {
    if (
      (torrent.error !== undefined && torrent.error > 0) ||
      torrent.cachedError !== ""
    ) {
      variant = "red";
    } else {
      if (torrent.status === Status.stopped && torrent.sizeWhenDone > 0) {
        ...
        torrent.status === Status.queuedToVerify ||
