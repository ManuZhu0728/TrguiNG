...
export default function CreateTorrentForm() {
    ...
    useEffect(() => {
        void appWindow.once<PassEventData>(
            "pass-from-window",
            ({ payload: data }) => {
    ...
    const setPathAndCalculate = useCallback(
        (path: string[] | string | null) => {
            ...
            invoke<GetFileStatsResult>("get_file_stats", { path })
                .then((sizes) => {
                    if (sizes.files === -1) {
                        setState({
                            state: "error",
                            error: t("createtorrent.error.calculateSizes"),
                            hash: "",
                        });
                        return;
                    }
    ...
    const onGenerate = useCallback(() => {
        invoke("create_torrent", { info: form.values })
            .then(() => {
                clearInterval(timer.current);

                timer.current = window.setInterval(() => {
                    invoke<CreateCheckResult>("check_create_torrent")
                        .then((result) => {
                            ...
                            if (result.notFound !== undefined) {
                                setState({
                                    state: "error",
                                    error: t("createtorrent.error.notFound"),
                                    hash: "",
                                });
                                clearInterval(timer.current);
                            }
                            if (result.inProgress !== undefined) {
                                setPieces({
                                    done: result.inProgress.hashed,
                                    total: result.inProgress.total,
                                });
                            }
                        })
                        .catch((error) => {
    ...
    const onSave = useCallback(() => {
        dialogSave({
            title: t("createtorrent.dialog.saveTorrentFile"),
            ...
            .then((path) => {
                if (path != null) {
                    invoke("save_create_torrent", { path })
                        .then(() => {
                            setState({ state: "idle", error: "", hash: "" });
                        })
                        .catch((error) => {
        ...
        <Flex direction="column" h="100%" w="100%" p="lg" gap="lg">
            <Group align="flex-end">
                <TextInput
                    ...
                    spellCheck="false"
                />
                <Button onClick={onBrowseFile} disabled={browseDisabled}>
                    {t("createtorrent.file")}
                </Button>
                <Button onClick={onBrowseDirectory} disabled={browseDisabled}>
                    {t("createtorrent.directory")}
                </Button>
            </Group>
            <TextInput
            ...
            <Slider
                ...
                onChange={(value) => {
                    form.setFieldValue("pieceLength", 2 ** value);
                }}
            />
            <TextInput
            ...
            <Group align="flex-end">
                ...
                <Button onClick={addDefaultTrackers}>
                    {t("createtorrent.addDefaultList")}
                </Button>
            </Group>
            <Textarea
                ...
                onChange={(e) => {
                    form.setFieldValue("announceList", e.target.value.split("\n"));
                }}
            />
            <Textarea
                styles={textAreaStyles}
                label={t("createtorrent.webSeeds")}
                value={form.values.urlList.join("\n")}
                onChange={(e) => {
                    form.setFieldValue("urlList", e.target.value.split("\n"));
                }}
            />
            <Box h="1.5rem">
                {state.state === "error" && <Text color="red">{state.error}</Text>}
                {state.state === "calculating" && (
                    <Text>{t("createtorrent.calculatingSizes")}</Text>
                )}
                {state.state === "sizes" && (
                    <Text>
                        {`${t("createtorrent.fileCount", {
                            count: state.sizes?.files ?? 1,
                        })}, `}
                        {`${bytesToHumanReadableStr(state.sizes?.size ?? 0)}, `}
                        {t("createtorrent.pieces", {
                            count: Math.ceil(
                                (state.sizes?.size ?? 0) / form.values.pieceLength,
                            ),
                        })}
                    </Text>
                )}
                ...
                {state.state === "done" && (
                    <Text>{t("createtorrent.infohash", { hash: state.hash })}</Text>
                )}
            </Box>
            <Group position="center">
                {["idle", "error", "calculating", "sizes"].includes(state.state) && (
                    <Button
                        miw="10rem"
                        onClick={onGenerate}
                        disabled={state.state === "calculating"}
                    >
                        {t("createtorrent.generate")}
                    </Button>
                )}
                {state.state === "generating" && (
                    <Button miw="10rem" onClick={onCancel} color="red">
                        {t("createtorrent.cancel")}
                    </Button>
                )}
                {state.state === "done" && (
                    <Button miw="10rem" onClick={onSave} color="green">
                        {t("createtorrent.save")}
                    </Button>
                )}
            </Group>
        </Flex>
