...
export function getSectionsMap<S extends string>(
...
function SectionsContextMenu<S extends string>(
  props: React.PropsWithChildren<{
    ...
    translationMap?: Record<S, string>;
    t?: (key: string) => string;
  }>
) {
  const { setSections } = props;

  const onSectionMenuItemClick = useCallback(
    (index: number) => {
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      if (result.destination != null) {
        const sections = reorderElements(
          props.sections,
          result.source.index,
                    ...
                    {(provided) => (
                      ...
                      >
                        <Menu.Item
                          ...
                          onClick={() => {
                            onSectionMenuItemClick(index);
                          }}
                        >
                          {props.t && props.translationMap
                            ? props.t(props.translationMap[section.section])
                            : section.section}
                        </Menu.Item>
...
export const MemoSectionsContextMenu = React.memo(
  SectionsContextMenu
) as typeof SectionsContextMenu;
