...
import { ConfigContext, type DetailsSectionsName } from "config";
...
function Wasted(props: { torrent: Torrent }) {
    const { t } = useTranslation();
    const hashfails =
    props.torrent.pieceSize > 0
        ? props.torrent.corruptEver / props.torrent.pieceSize
        : 0;
    return (
        <>{`${bytesToHumanReadableStr(props.torrent.corruptEver)} (${hashfails} ${t(
            "details.wasted.hashfails",
        )})`}</>
    );
}
...
function DownloadSpeed(props: { torrent: Torrent }) {
    ...
    if (secondsDownloading > 0) {
        return (
            <>{`${speed} (${t(
                "details.downloadSpeed.average",
            )}: ${bytesToHumanReadableStr(
                props.torrent.downloadedEver / secondsDownloading,
            )}/s)`}</>
        );
    } else {
        return <>{speed}</>;
...
function Seeds(props: { torrent: Torrent }) {
    ...
    if (totalSeeds < 0) {
        ...
        return (
            <>{`${sending} ${t("details.seeds.of")} ${totalSeeds} ${t(
                "details.seeds.connected",
            )}`}</>
        );
    }
}
...
function Peers(props: { torrent: Torrent }) {
    ...
    if (totalPeers < 0) {
        ...
        return (
            <>{`${getting} ${t("details.peers.of")} ${totalPeers} ${t(
                "details.peers.connected",
            )}`}</>
        );
    }
}
...
function TrackerUpdate(props: { torrent: Torrent }) {
    ...
    return (
        <>
            {state === 2 || state === 3
                ? "-"
                : timestampToDateString(tracker.nextAnnounceTime, config)}
        </>
    );
}
...
function TransferTable(props: { torrent: Torrent }) {
    ...
    const shareRatio = `${(props.torrent.uploadRatio as number).toFixed(5)} ${
    ...
    return (
        <Container fluid>
            ...
            >
                <DetailItem name={t("details.transfer.status")}>
                    <StatusField {...props} fieldName="status" />
                </DetailItem>
                <DetailItem name={t("details.transfer.error")}>
                    {props.torrent.cachedError}
                </DetailItem>
                <DetailItem
                    name={t("details.transfer.remaining")}
                >{`${secondsToHumanReadableStr(
                        props.torrent.eta,
                    )} (${bytesToHumanReadableStr(
                        props.torrent.leftUntilDone,
                    )})`}</DetailItem>
                <DetailItem name={t("details.transfer.downloaded")}>
                    {bytesToHumanReadableStr(props.torrent.downloadedEver)}
                </DetailItem>
                <DetailItem name={t("details.transfer.uploaded")}>
                    {bytesToHumanReadableStr(props.torrent.uploadedEver)}
                </DetailItem>
                <DetailItem name={t("details.transfer.wasted")}>
                    <Wasted {...props} />
                </DetailItem>
                <DetailItem name={t("details.transfer.downloadSpeed")}>
                    <DownloadSpeed {...props} />
                </DetailItem>
                <DetailItem
                    name={t("details.transfer.uploadSpeed")}
                >{`${bytesToHumanReadableStr(props.torrent.rateUpload)}/s`}</DetailItem>
                <DetailItem name={t("details.transfer.shareRatio")}>
                    {shareRatio}
                </DetailItem>
                <DetailItem name={t("details.transfer.downloadLimit")}>
                    <SpeedLimit {...props} field="download" />
                </DetailItem>
                <DetailItem name={t("details.transfer.uploadLimit")}>
                    <SpeedLimit {...props} field="upload" />
                </DetailItem>
                <DetailItem name={t("details.transfer.bandwidthGroup")}>
                    {props.torrent.group}
                </DetailItem>
                <DetailItem name={t("details.transfer.seeds")}>
                    <Seeds {...props} />
                </DetailItem>
                <DetailItem name={t("details.transfer.peers")}>
                    <Peers {...props} />
                </DetailItem>
                <DetailItem name={t("details.transfer.maxPeers")}>
                    {props.torrent.maxConnectedPeers}
                </DetailItem>
                <DetailItem name={t("details.transfer.tracker")}>
                    <TrackerField {...props} fieldName="trackerStats" />
                </DetailItem>
                <DetailItem name={t("details.transfer.trackerUpdateOn")}>
                    <TrackerUpdate {...props} />
                </DetailItem>
                <DetailItem name={t("details.transfer.lastActive")}>
                    <DateField {...props} fieldName="activityDate" />
                </DetailItem>
            </Grid>
        </Container>
    );
...
function TotalSize(props: { torrent: Torrent }) {
    ...
    const done = bytesToHumanReadableStr(
        props.torrent.sizeWhenDone - props.torrent.leftUntilDone,
    );
    return <>{`${size} (${done} ${t("details.totalSize.done")})`}</>;
}
...
function Pieces(props: { torrent: Torrent }) {
    ...
    if (props.torrent.totalSize === props.torrent.haveValid) {
        ...
        have =
      props.torrent.haveValid /
      (props.torrent.pieceSize > 0 ? props.torrent.pieceSize : 1);
    }

    return (
        <>{`${props.torrent.pieceCount as number} x ${pieceSize} (${t(
            "details.pieces.have",
        )} ${Math.round(have)})`}</>
    );
}
    ...
    return (
        <>
            {matches.map((match, index) => {
                ...
                return (
                    <Anchor key={index} href={match} target="_blank" rel="noreferrer">
                        {match}
                    </Anchor>
                );
            })}
...
function TorrentDetails(props: { torrent: Torrent }) {
    ...
    const fullPath =
    ensurePathDelimiter(props.torrent.downloadDir) +
    fileSystemSafeName(props.torrent.name);
    ...
    return (
        <Container fluid>
            ...
            >
                <DetailItem name={t("details.torrent.fullPath")}>
                    <TextInput
                ...
                <DetailItem name={t("details.torrent.created")}>
                    <div>
                        <span>
                        ...
                        <span>
                            {props.torrent.creator === undefined ||
              props.torrent.creator === ""
                                ? ""
                                : ` ${t("details.torrent.by")} ${
                                    props.torrent.creator as string
                                }`}
                        </span>
                    </div>
                </DetailItem>
                <DetailItem name={t("details.torrent.totalSize")}>
                    <TotalSize {...props} />
                </DetailItem>
                <DetailItem name={t("details.torrent.pieces")}>
                    <Pieces {...props} />
                </DetailItem>
                <DetailItem name={t("details.torrent.hash")}>
                    <TextInput
                        styles={readonlyInputStyles}
                        variant="unstyled"
                        readOnly
                ...
                <DetailItem name={t("details.torrent.comment")}>
                    <Urlize text={props.torrent.comment} />
                </DetailItem>
                <DetailItem name={t("details.torrent.addedOn")}>
                    <DateField {...props} fieldName="addedDate" />
                </DetailItem>
                <DetailItem name={t("details.torrent.completedOn")}>
                    <DateField {...props} fieldName="doneDate" />
                </DetailItem>
                <DetailItem name={t("details.torrent.magnetLink")}>
                    <TextInput
                        styles={readonlyInputStyles}
                        variant="unstyled"
                        readOnly
                ...
                <DetailItem name={t("details.torrent.labels")}>
                    <LabelsField {...props} fieldName="labels" />
                </DetailItem>
            </Grid>
        </Container>
    );
        ...
        <Flex direction="column" h="100%" w="100%">
            ...
            <div style={{ flexGrow: 1 }}>
                <div className="scrollable">
                    <Container fluid>
                        <TableNameRow>{t("details.section.transfer")}</TableNameRow>
                        <TransferTable {...props} />
                        <TableNameRow>{t("details.section.torrent")}</TableNameRow>
...
function FileTreePane(props: { torrent: Torrent }) {
    ...
    const updateUnwanted = useCallback(
        (entryPath: string, state: boolean) => {
            onCheckboxChange(entryPath, state);
            mutate({
                torrentIds: [props.torrent.id],
                fields: {
                    [state ? "files-wanted" : "files-unwanted"]:
            fileTree.getChildFilesIndexes(entryPath),
                },
            });
        },
...
function Stats(props: { stats: SessionStatEntry }) {
    const { t } = useTranslation();
    return (
        <Table mb="sm" sx={{ maxWidth: "25em" }}>
            <tbody>
                <tr>
                    <td style={{ width: "10em" }}>{t("details.stats.downloaded")}</td>
                    <td>{bytesToHumanReadableStr(props.stats.downloadedBytes)}</td>
                </tr>
                <tr>
                    <td>{t("details.stats.uploaded")}</td>
                    <td>{bytesToHumanReadableStr(props.stats.uploadedBytes)}</td>
                </tr>
                <tr>
                    <td>{t("details.stats.ratio")}</td>
                    <td>
                        {props.stats.downloadedBytes === 0
                            ? "∞"
                            : (
                                props.stats.uploadedBytes / props.stats.downloadedBytes
                            ).toFixed(2)}
                    </td>
                </tr>
                <tr>
                    <td>{t("details.stats.filesAdded")}</td>
                    <td>{props.stats.filesAdded}</td>
                </tr>
                <tr>
                    <td>{t("details.stats.active")}</td>
                    <td>{secondsToHumanReadableStr(props.stats.secondsActive)}</td>
                </tr>
                {props.stats.sessionCount > 1 && (
                    <tr>
                        <td>{t("details.stats.sessionCount")}</td>
            ...
            <div style={{ flexGrow: 1 }}>
                <div className="scrollable">
                    {sessionStats !== undefined ? (
                        <Container fluid>
                            <TableNameRow>{t("details.section.session")}</TableNameRow>
                            <Stats stats={sessionStats["current-stats"]} />
                            <TableNameRow>{t("details.section.cumulative")}</TableNameRow>
...
const DetailsPanels = React.memo(function DetailsPanels({
    torrent,
}: {
    ...
    return (
        <>
            <Tabs.Panel value="General" h="100%">
                {torrent !== undefined ? <GeneralPane torrent={torrent} /> : <></>}
            </Tabs.Panel>
            <Tabs.Panel value="Files" h="100%">
                {torrent !== undefined ? <FileTreePane torrent={torrent} /> : <></>}
            </Tabs.Panel>
            <Tabs.Panel value="Pieces" h="100%">
                {torrent !== undefined ? <PiecesCanvas torrent={torrent} /> : <></>}
            </Tabs.Panel>
            <Tabs.Panel value="Peers" h="100%">
                {torrent !== undefined ? <PeersTable torrent={torrent} /> : <></>}
            </Tabs.Panel>
            <Tabs.Panel value="Trackers" h="100%">
                {torrent !== undefined ? <TrackersTable torrent={torrent} /> : <></>}
            </Tabs.Panel>
...
function Details(props: DetailsProps) {
    ...
    const { data: fetchedTorrent, isLoading } = useTorrentDetails(
        props.torrentId ?? -1,
        props.torrentId !== undefined && props.updates,
        countryVisible,
    );
    ...
    const sectionTranslationMap: Record<DetailsSectionsName, string> = {
        General: "details.tabs.general",
        Files: "details.tabs.files",
        Pieces: "details.tabs.pieces",
        Peers: "details.tabs.peers",
        Trackers: "details.tabs.trackers",
        "<spacer>": "details.tabs.spacer",
        "Server statistics": "details.tabs.serverStatistics",
    };
        ...
        >
            <Tabs.List px="sm" pt="xs" onContextMenu={handler}>
                ...
                {tabs[tabsMap.General].visible && (
                    <Tabs.Tab
                        value="General"
                        disabled={torrent === undefined}
                        style={{ order: tabsMap.General }}
                    >
                        <Group>
                ...
                {tabs[tabsMap.Files].visible && (
                    <Tabs.Tab
                        value="Files"
                        disabled={torrent === undefined}
                        style={{ order: tabsMap.Files }}
                    >
                        <Group>
                            <Icon.Files size="1.1rem" />
                            {`${t("details.tabs.files")}${
                                torrent !== undefined
                                    ? ` (${torrent.files.length as number})`
                                    : ""
                            }`}
                        </Group>
                    </Tabs.Tab>
                ...
                {tabs[tabsMap.Pieces].visible && (
                    <Tabs.Tab
                        value="Pieces"
                        disabled={torrent === undefined}
                        style={{ order: tabsMap.Pieces }}
                    >
                        <Group>
                            <Icon.Grid3x2 size="1.1rem" />
                            {`${t("details.tabs.pieces")}${
                                torrent !== undefined
                                    ? ` (${torrent.pieceCount as number})`
                                    : ""
                            }`}
                        </Group>
                    </Tabs.Tab>
                ...
                {tabs[tabsMap.Peers].visible && (
                    <Tabs.Tab
                        value="Peers"
                        disabled={torrent === undefined}
                        style={{ order: tabsMap.Peers }}
                    >
                        <Group>
                            <Icon.People size="1.1rem" />
                            {t("details.tabs.peers")}
                        </Group>
                    </Tabs.Tab>
                )}
                {tabs[tabsMap.Trackers].visible && (
                    <Tabs.Tab
                        value="Trackers"
                        disabled={torrent === undefined}
                        style={{ order: tabsMap.Trackers }}
                    >
                        <Group>
                            <Icon.Wifi size="1.1rem" />
                            {t("details.tabs.trackers")}
                        </Group>
                    </Tabs.Tab>
                )}
                ...
                {tabs[tabsMap["Server statistics"]].visible && (
                    <Tabs.Tab
                        ...
                        style={{ order: tabsMap["Server statistics"] }}
                    >
                        <Group>
                            <Icon.ArrowDownUp size="1.1rem" />
                            {t("details.tabs.serverStatistics")}
                        </Group>
                    </Tabs.Tab>
                )}
            </Tabs.List>
