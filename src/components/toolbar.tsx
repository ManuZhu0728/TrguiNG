...
export const ToolbarButton = forwardRef<HTMLButtonElement, ToolbarButtonProps>(
  function ToolbarButton(
    { children, depressed, ...other }: ToolbarButtonProps,
    ref
  ) {
    return (
      <Button
...
function useButtonHandlers(
  ...
  const toggleAltSpeedMode = useCallback(() => {
    sessionMutation.mutate(
      { "alt-speed-enabled": altSpeedMode !== true },
      {
        onError: (_, session) => {
...
function Toolbar(props: ToolbarProps) {
  ...
  const [searchPlaceholder, setSearchPlaceholder] = useState<string>(
    t("toolbar.searchPlaceholder", { hotkey: modKeyString() })
  );
  ...
  const onSettingsExport = useCallback(() => {
    void saveJsonFile(
      config.getExportedInterfaceSettings(),
      "trguing-interface.json"
    );
  }, [config]);
  ...
  return (
    <Flex w="100%" align="stretch">
      <Button.Group mx="sm">
        <ToolbarButton
          title={t("toolbar.addTorrentFile")}
          onClick={() => {
      ...
      <Button.Group mx="sm">
        <ToolbarButton title={t("toolbar.moveTorrent")} onClick={handlers.move}>
          <Icon.FolderFill
            size="1.5rem"
        ...
        >
          ...
          <Menu.Dropdown>
            <Menu.Item
              icon={<Icon.CircleFill color={theme.colors.orange[7]} />}
            ...
            <Menu.Item
              icon={<Icon.CircleFill color={theme.colors.teal[9]} />}
            ...
            <Menu.Item
              icon={<Icon.CircleFill color={theme.colors.yellow[6]} />}
      ...
      <ToolbarButton
        title={
          altSpeedMode === true
            ? t("toolbar.altSpeedOff")
            : t("toolbar.altSpeedOn")
        }
      ...
      <TextInput
        ...
        rightSection={
          <ActionIcon onClick={onSearchClear} title={t("toolbar.clearSearch")}>
            <Icon.XLg size="1rem" color={theme.colors.red[6]} />
          </ActionIcon>
        }
        onInput={onSearchInput}
        onFocus={() => setSearchPlaceholder(t("toolbar.searchHint"))}
        onBlur={() =>
      ...
      >
        <Menu.Target>
          <ToolbarButton title={t("toolbar.layoutMenu")}>
            <Icon.Grid1x2Fill
              size="1.5rem"
        ...
        <Menu.Dropdown>
          ...
          <Menu.Item
            ...
            rightSection={<Kbd>{`${modKeyString()} O`}</Kbd>}
          >
          ...
          <Menu.Item onClick={onSettingsExport}>
            {t("toolbar.exportSettings")}
          </Menu.Item>
          <Menu.Item
            onClick={() => {
              void onSettingsImport();
            }}
          >
            {t("toolbar.importSettings")}
          </Menu.Item>
        </Menu.Dropdown>
      </Menu>

      <ToolbarButton
        title={t("toolbar.daemonSettings")}
