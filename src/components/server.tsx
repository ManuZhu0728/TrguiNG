...
import {
    ServerRpcVersionContext,
    ServerSelectedTorrentsContext,
    ServerTorrentDataContext,
} from "../rpc/torrent";
...
import {
    TorrentTable,
    useInitialTorrentRequiredFields,
...
function ServerContext(props: ServerContextProps) {
    return (
        <ServerTorrentDataContext.Provider value={props.data}>
...
export function Server({
    ...
    const [currentFilters, setCurrentFilters] = useReducer(
        currentFiltersReducer,
        [{ id: "", filter: DefaultFilter }],
    );
    ...
    const searchFilter = useCallback(
        (t: Torrent) => {
            ...
            const labels = (
                (t.labels ?? []).map((l: string) => l.toLowerCase()) as string[]
            ).join(" ");
    ...
    const [tableRequiredFields, setTableRequiredFields] = useState<
        TorrentFieldsType[]
    >(useInitialTorrentRequiredFields());
    ...
    useEffect(() => {
        if ((torrents?.findIndex((t) => t.id === currentTorrent) ?? -1) === -1)
            setCurrentTorrentInt(undefined);
    ...
    useEffect(() => {
        if (currentTorrent !== undefined)
            setScrollToRow({ id: `${currentTorrent}` });
    ...
    const [showFiltersPanel, { toggle: toggleFiltersPanel }] = useDisclosure(
        config.values.interface.showFiltersPanel,
    );
    const [showDetailsPanel, { toggle: toggleDetailsPanel }] = useDisclosure(
        config.values.interface.showDetailsPanel,
    );
    ...
    return (
        ...
        >
            <Flex direction="column" w="100%" h="100%" sx={{ position: "relative" }}>
                    ...
                    <Overlay blur={10}>
                        ...
                        >
                            {sessionIsLoading ? (
                                ...
                                <>
                                    <Title color="red" order={1}>
                                        {t("server.error.loadSession")}
                                    </Title>
                                ...
                                <Title color="red" order={1}>
                                    {t("server.error.notTransmission")}
                                </Title>
                            ) : rpcVersion < 14 ? (
                                <Title color="red" order={1}>
                                    {t("server.error.versionRequired")}
                                </Title>
                            ) : (
                                <></>
                            )}
                ...
                <SplitLayout
                    key={`split-${showFiltersPanel ? "1" : "0"}-${
                        showDetailsPanel ? "1" : "0"
                    }-${mainSplit}`}
                    mainSplit={mainSplit}
                    left={
                        showFiltersPanel ? (
