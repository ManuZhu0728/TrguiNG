...
export function LimitedNamesList({
  ...
  const { t } = useTranslation();
  limit = limit ?? 5;
  const t_names = names.slice(0, limit);
  ...
  return (
    <>
      ...
      {names.length > limit && (
        <Text mx="xl" mb="md">
          {t("modals.common.and_more", { count: names.length - limit })}
        </Text>
      )}
    </>
  );
...
export function TorrentsNames() {
  ...
  const allNames = useMemo<string[]>(() => {
    if (serverData.current == null || serverSelected.size === 0) {
      return [t("modals.common.no_torrent_selected")];
    }
...
export function useTorrentLocation(): LocationData {
  ...
  const browseHandler = useCallback(() => {
    ...
    dialogOpen({
      title: t("modals.common.select_directory"),
                ...
                {props.lastPaths.map((path) => (
                  <Menu.Item
                    ...
                    rightSection={
                      <ActionIcon
                        component="div"
                        title={t("modals.common.remove_path")}
                        onClick={(e) => {
                          e.stopPropagation();
...
export function TorrentLabels(props: TorrentLabelsProps) {
  ...
  return (
    <MultiSelect
      ...
      disabled={props.disabled}
      getCreateLabel={(query) => t("modals.common.add_label", { query })}
      onCreate={(query) => {
