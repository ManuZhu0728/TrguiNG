...
export function EditTrackers(props: ModalState) {
  ...
  const onSave = useCallback(() => {
    ...
    for (const tracker of form.values.trackerList.split("\n")) {
      ...
      if (tracker === "" && dedupedTrackers[dedupedTrackers.length - 1] === "")
        continue;
    ...
    if (rpcVersion < 17) {
      ...
      toAdd = trackers.filter(
        (t) => !Object.prototype.hasOwnProperty.call(currentTrackers, t)
      );
    ...
    mutate(
      ...
      },
      {
        onError: (e) => {
          ...
          notifications.show({
            message: t("modals.editTrackers.errorUpdatingTorrent"),
            color: "red",
          });
        },
      }
    ...
    <>
      {props.opened && (
        <SaveCancelModal
          ...
          onSave={onSave}
          centered
          title={t("modals.editTrackers.editTorrentTrackers")}
          mih="25rem"
        >
          ...
          <Grid align="center">
            ...
            <Grid.Col span={8}>
              <Text>{t("modals.editTrackers.trackerListHint")}</Text>
            </Grid.Col>
            <Grid.Col span={4}>
              <Button onClick={addDefaultTrackers}>
                {t("modals.editTrackers.addDefaultList")}
              </Button>
            </Grid.Col>
            <Grid.Col>
              <Textarea minRows={10} {...form.getInputProps("trackerList")} />
            </Grid.Col>
          </Grid>
        </SaveCancelModal>
      )}
