...
import type {
  ServerConfig,
  WindowCloseOption,
  WindowMinimizeOption,
} from "config";
import {
  ConfigContext,
  WindowCloseOptions,
  WindowMinimizeOptions,
} from "config";
import React, {
  useCallback,
  useContext,
...
function ServerPanel(props: ServerPanelProps) {
  ...
  useEffect(() => {
    setMappingsString(
      server.pathMappings.map((m) => `${m.from}=${m.to}`).join("\n")
    );
  }, [server.pathMappings]);
          ...
          <Textarea
            ...
            onChange={(e) => {
              ...
              props.form.setFieldValue(
                `servers.${props.current}.pathMappings`,
                mappings
              );
...
function IntegrationsPanel({ form }: { form: UseFormReturnType<FormValues> }) {
  ...
  const onChangeAutostart = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const state = e.target.checked;
      setAutostart(state);
      void invoke("app_integration", {
        mode: state ? "autostart" : "noautostart",
      });
    },
    []
  );
    ...
    <Grid align="center" gutter="md">
      ...
      <Grid.Col span={2}>
        <Switch
          ...
          size="xl"
          styles={bigSwitchStyles}
          {...form.getInputProps("app.toastNotifications", {
            type: "checkbox",
          })}
        />
      </Grid.Col>
      ...
      <Grid.Col span={2}>
        <Switch
          ...
          size="xl"
          styles={bigSwitchStyles}
          {...form.getInputProps("app.toastNotificationSound", {
            type: "checkbox",
          })}
        />
      </Grid.Col>
          ...
          <Tabs.Panel value="servers" pt="md">
            <Flex h="100%" gap="0.5rem" mih="28rem">
              <ServerListPanel
                ...
                setCurrent={setCurrentServerIndex}
              />
