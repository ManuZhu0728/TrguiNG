...
import {
  FileTreeTable,
  useUnwantedFiles,
} from "components/tables/filetreetable";
...
function AddCommon(props: AddCommonProps) {
  ...
  return (
    <>
      <TorrentLocation
        {...props.location}
        inputLabel={t("modals.add.download_dir")}
      ...
      {rpcVersion >= 17 && (
        <TorrentLabels
          ...
          inputLabel={t("modals.add.labels")}
          disabled={props.disabled}
        />
      )}
      <Group>
        <Checkbox
          label={t("modals.add.start_torrent")}
          checked={props.start}
          disabled={props.disabled}
          onChange={(e) => {
            props.setStart(e.currentTarget.checked);
          }}
          my="xl"
        ...
        <SegmentedControl
          ...
          onChange={(value) => {
            props.setPriority(+value as PriorityNumberType);
          }}
          disabled={props.disabled}
  ...
  useEffect(() => {
    if (opened) {
      ...
      if (config.values.interface.addTorrentPriority === "remember selection") {
        ...
        setPriority(
          (AddTorrentPriorityOptions.indexOf(
            config.values.interface.addTorrentPriority
          ) - 1) as PriorityNumberType
        );
      }
    }
  }, [config, opened]);
...
function TabSwitchDropdown({
  ...
  const onChange = useCallback(
    (value: string) => {
      setValue(value);
      const tabIndex = tabsRef.current
        ?.getOpenTabs()
        .findIndex((v) => v === value);
  ...
  return tabsRef.current == null ? (
    <></>
  ) : (
    <Menu shadow="md" width={200} position="bottom-start">
      <Menu.Target>
        <Button variant="subtle" title={t("modals.add.switch_server")}>
          {value}
        </Button>
      </Menu.Target>
...
export function AddMagnet(props: AddCommonModalProps) {
  ...
  useEffect(() => {
    if (magnetData !== undefined && magnetData.hash !== "") {
      const torrent = serverData.torrents.find(
        (t) => t.hashString === magnetData.hash
      );
  ...
  const addMutation = useAddTorrent(
    ...
    useCallback(
      (response: any) => {
        ...
        if (duplicate !== undefined) {
          notifications.show({
            title: t("modals.add.torrent_already_exists"),
        ...
        if (added !== undefined) {
          notifications.show({
            title: t("modals.add.torrent_added"),
            message: added.name,
            color: "green",
          });
        }
    ...
    useCallback(
      (e) => {
        ...
        notifications.show({
          title: t("modals.add.error_adding_torrent"),
          message: String(e),
          color: "red",
        });
      },
    ...
    if (existingTorrent === undefined) {
      ...
      mutateAddTrackers(
        { torrentId: existingTorrent.id, trackers: magnetData?.trackers ?? [] },
        {
          onSuccess: () => {
            notifications.show({
              message: t("modals.add.trackers_updated"),
              color: "green",
            });
          },
        }
  ...
  const config = useContext(ConfigContext);
  const shouldOpen =
    !config.values.interface.skipAddDialog || typeof props.uri !== "string";
  useEffect(() => {
      ...
      {props.opened && shouldOpen && (
        <HkModal
          ...
          title={
            <Flex w="100%" align="center" justify="space-between">
              <span>{t("modals.add.add_magnet_title")}</span>
        ...
        >
          ...
          <TextInput
            ...
            error={
              existingTorrent === undefined
                ? undefined
                : t("modals.add.torrent_already_exists")
            }
            autoComplete="off"
            autoCorrect="off"
            autoCapitalize="off"
          ...
          <AddCommon
            {...common.props}
            disabled={existingTorrent !== undefined}
          />
          <Divider my="sm" />
          <Group position="center" spacing="md">
            <Button
              ...
              disabled={
                existingTorrent !== undefined &&
                (magnetData?.trackers.length ?? 0) === 0
              }
            >
              {existingTorrent === undefined
                ? t("modals.add.add_button")
                : t("modals.add.add_trackers_button")}
            </Button>
...
async function readLocalTorrent(file: File): Promise<string> {
  return await new Promise((resolve) => {
    ...
    reader.onloadend = () => {
      const b64 = (reader.result as string).match(
        /data:[^/]*\/[^;]*;base64,(.*)/
      )?.[1];
      if (b64 === undefined) {
...
function useTauriReadFile(
  ...
  setTorrentData: React.Dispatch<
    React.SetStateAction<TorrentFileData[] | undefined>
  >
) {
  const { t } = useTranslation();
  useEffect(() => {
    if (TAURI && props.opened && torrentData === undefined) {
      const readFile = async (path: string | string[] | null) => {
        ...
        if (Array.isArray(path)) {
          return await Promise.all(
            path.map(
              async (p) =>
                await invoke<TorrentFileData>("read_file", { path: p })
            )
          );
        }
      ...
      const pathPromise =
        typeof uri === "string"
          ? Promise.resolve(uri)
          : dialogOpen({
              title: t("modals.add.select_torrent_file"),
              filters: [
                {
                  name: "Torrent",
                  extensions: ["torrent"],
      ...
      pathPromise
        ...
        .catch((e) => {
          notifications.show({
            title: t("modals.add.error_reading_torrent"),
            message: String(e),
            color: "red",
          });
          props.close();
  ...
  }, [props, setTorrentData, torrentData, t]);
}
...
function useWebappReadFile(
    ...
    if (!TAURI && props.opened) {
      if (props.uri === undefined) {
        ...
        readLocalTorrent(file)
          ...
          .catch(() => {
            notifications.show({
              title: t("modals.add.error_reading_file"),
              message: file.name,
              color: "red",
            });
            close();
  ...
  }, [props.opened, props.uri, close, filesInputRef, setTorrentData, t]);
}
...
function useFilesInput(
  ...
  useEffect(() => {
    ...
    const fileInputListener = (e: Event) => {
      ...
      if (element.files == null) {
        ...
        Promise.all(files.map(readLocalTorrent))
          .then((filesData) => {
            setTorrentData(
              filesData.map((b64, i) => ({
                torrentPath: "",
  ...
  }, [close, filesInputRef, setTorrentData, t]);
}
...
export function AddTorrent(props: AddCommonModalProps) {
  ...
  const existingTorrent = useMemo(() => {
    if (torrentData !== undefined && torrentData.length === 1) {
      return serverData.torrents.find(
        (t) => t.hashString === torrentData[0].hash
      );
    }
    return undefined;
  ...
  const fileTree = useMemo(
    () => new CachedFileTree(torrentData?.[0]?.hash ?? "", -1),
    [torrentData]
  );
  ...
  useEffect(() => {
    ...
    if (torrentData.length === 1 && torrentData[0].files != null) {
      ...
      const totalSize = torrentData.reduce(
  ...
  const setAllWanted = useCallback(
    (wanted: boolean) => {
        ...
        { torrentId: existingTorrent.id, trackers: torrentData[0].trackers },
        {
          ...
          onSettled: () => {
            if (
              TAURI &&
              config.values.app.deleteAdded &&
              torrentData[0].torrentPath !== undefined
            ) {
  ...
  const shouldOpen =
    !config.values.interface.skipAddDialog && torrentData !== undefined;
  useEffect(() => {
      ...
      {shouldOpen && (
        <HkModal
          ...
          title={
            <Flex w="100%" align="center">
              <span>{t("modals.add.add_torrent_title")}</span>
              <Box sx={{ flexGrow: 1 }} />
              {TAURI && (
                <>
        ...
        >
            ...
            {torrentData.length > 1 || torrentData[0].files == null ? (
              ...
              <>
                <Button
                  ...
                  title={t("modals.add.mark_all_wanted")}
                >
                  {t("modals.add.all")}
                </Button>
                <Button
                  ...
                  title={t("modals.add.mark_all_unwanted")}
                >
                  {t("modals.add.none")}
                </Button>
              </>
          ...
          <Group position="center" spacing="md">
            ...
            <Button onClick={modalClose} variant="light">
              {t("modals.common.cancel")}
            </Button>
          </Group>
        </HkModal>
