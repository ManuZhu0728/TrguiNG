...
export const ServerTabs = React.forwardRef<ServerTabsRef, ServerTabsProps>(
    function ServerTabs(props, ref) {
        ...
        useEffect(() => {
            const pollerConfigs = tabs.openTabs
                .filter((_, i) => i !== tabs.currentTab)
                .map((t) => {
            ...
            [
                ...
                useCallback(
                    (e) => {
                        e.preventDefault();
                        switchTab(
                            (tabs.currentTab + tabs.openTabs.length - 1) %
        ...
        const openTab = useCallback(
            (name: string) => {
                ...
                setTabs({
                    ...tabs,
                    openTabs: [...tabs.openTabs, name],
                    currentTab: tabs.openTabs.length,
                });
            },
        ...
        const closeTab = useCallback(
            (tab: number) => {
                ...
                if (
                    tabs.currentTab > tab ||
          (tabs.currentTab === tab &&
            newTabs.currentTab === tabs.openTabs.length - 1)
                ) {
                    newTabs.currentTab -= 1;
                }

                setCurrentServer(
                    config.getServer(newTabs.openTabs[newTabs.currentTab]),
                );
                setTabs(newTabs);
            },
            [tabs, props.clientManager, setCurrentServer, config],
        ...
        const unopenedTabs = useMemo(() => {
            return props.servers
                .filter((s) => !tabs.openTabs.includes(s.name))
                .map((s) => s.name);
        ...
        onServersChange.current = useCallback(
            (servers: ServerConfig[]) => {
                ...
                <Tabs.List px="sm">
                    ...
                    {unopenedTabs.length > 0 ? (
                        <Menu shadow="md" width={200} position="bottom-start">
                            ...
                            <Menu.Dropdown>
                                <Menu.Label>{t("servertabs.connect")}</Menu.Label>
