...
import { EditableNameField, TableSelectReducer } from "./common";
import { TrguiTable } from "./common";
import {
  Badge,
  Box,
  Button,
...
interface TableFieldSimple {
  ...
  label: string;
  component:
    | React.FunctionComponent<TableFieldProps>
    | React.NamedExoticComponent<TableFieldProps>;
...
const TimeField = memo(
  function TimeField(props: TableFieldProps) {
    if (props.fieldName in props.torrent) {
      return (
        <div>{secondsToHumanReadableStr(props.torrent[props.fieldName])}</div>
      );
    } else {
      return <></>;
    }
...
const AllFields: readonly TableField[] = [
  ...
  { name: "totalSize", label: "tables.headers.size", component: ByteSizeField },
  {
    name: "sizeWhenDone",
    label: "tables.headers.sizeToDownload",
    component: ByteSizeField,
  },
  {
    name: "leftUntilDone",
    label: "tables.headers.sizeLeft",
    component: ByteSizeField,
  },
  { name: "haveValid", label: "tables.headers.have", component: ByteSizeField },
  {
    name: "downloadedEver",
    label: "tables.headers.downloaded",
    component: ByteSizeField,
  },
  {
    name: "uploadedEver",
    label: "tables.headers.uploaded",
    component: ByteSizeField,
  },
  {
    name: "uploadedEver",
    label: "tables.headers.simpleRatio",
  ...
  {
    name: "rateDownload",
    label: "tables.headers.downSpeed",
    component: ByteRateField,
  },
  {
    name: "rateUpload",
    label: "tables.headers.upSpeed",
    component: ByteRateField,
  },
  { name: "status", label: "tables.headers.status", component: StatusField },
  { name: "addedDate", label: "tables.headers.addedOn", component: DateField },
  {
    name: "peersSendingToUs",
    label: "tables.headers.seeds",
  ...
  },
  {
    ...
    label: "tables.headers.peers",
    component: PeersField,
    columnId: "peersGettingFromUs",
    accessorFn: (t) => t.peersGettingFromUs * 1e6 + t.cachedPeersTotal,
  },
  { name: "eta", label: "tables.headers.eta", component: EtaField },
  {
    name: "uploadRatio",
    label: "tables.headers.ratio",
    component: FixedDecimalField,
  },
  {
    name: "trackerStats",
    label: "tables.headers.tracker",
  ...
  {
    name: "trackerStats",
    label: "tables.headers.trackerStatus",
  ...
  {
    name: "trackerStats",
    label: "tables.headers.trackerDownloads",
  ...
  {
    name: "doneDate",
    label: "tables.headers.completedOn",
    component: DateField,
  },
  {
    name: "activityDate",
    label: "tables.headers.lastActive",
    component: DateDiffField,
  },
  { name: "downloadDir", label: "tables.headers.path", component: StringField },
  {
    name: "bandwidthPriority",
    label: "tables.headers.priority",
    component: PriorityField,
  },
  { name: "id", label: "tables.headers.id", component: PositiveNumberField },
  {
    name: "queuePosition",
    label: "tables.headers.queuePosition",
    component: PositiveNumberField,
  },
  {
    name: "secondsSeeding",
    label: "tables.headers.seedingTime",
    component: TimeField,
  },
  {
    name: "isPrivate",
    label: "tables.headers.private",
    component: StringField,
  },
  { name: "labels", label: "modals.add.labels", component: LabelsField },
  {
    name: "group",
    label: "tables.headers.bandwidthGroup",
    component: StringField,
  },
  {
    name: "file-count",
    label: "tables.headers.fileCount",
    component: PositiveNumberField,
  },
  {
    name: "pieceCount",
    label: "tables.headers.pieceCount",
    component: PositiveNumberField,
  },
  {
    name: "metadataPercentComplete",
    label: "tables.headers.metadata",
    component: PercentBarField,
  },
] as const;
...
function NameField(props: TableFieldProps) {
  ...
  if (
    props.torrent.status === Status.stopped &&
  ...
  if (
    (props.torrent.error !== undefined && props.torrent.error > 0) ||
  ...
  const currentName = useMemo(
    ...
    [props.fieldName, props.torrent]
  );

  const mutation = useMutateTorrentPath();

  const updateTorrentName = useCallback(
    (name: string, onStart: () => void, onEnd: () => void) => {
      onStart();
        ...
        mutation.mutate(
          { torrentId: props.torrent.id, path, name },
          {
            ...
            onError: () => {
              notifications.show({
                color: "red",
                message: "contextMenu.failedToRename",
              });
            },
          }
        );
...
function StringField(props: TableFieldProps) {
  return <div>{props.torrent[props.fieldName]}</div>;
}
  ...
  return (
    <div style={{ width: "100%", textAlign: "right" }}>
      {props.torrent.uploadedEver === 0
        ...
        ? "∞"
        : (props.torrent.uploadedEver / props.torrent.downloadedEver).toFixed(
            2
          )}
    </div>
  );
...
function TrackerDownloadsField(props: TableFieldProps) {
  return (
    <div style={{ width: "100%", textAlign: "right" }}>
      {props.torrent.cachedTrackerDlCount >= 0
        ? props.torrent.cachedTrackerDlCount
        : ""}
    </div>
  );
}
...
function PriorityField(props: TableFieldProps) {
  ...
  return (
    <Badge radius="md" variant="filled" bg={PriorityColors.get(priority)}>
      {t(PriorityStrings.get(priority) ?? "")}
    </Badge>
  );
}
...
export function StatusField(props: TableFieldProps) {
  const { t } = useTranslation();
  let status: string = t(StatusStrings[props.torrent.status]);
  if (
    props.torrent.status === Status.downloading &&
    props.torrent.pieceCount === 0
  )
...
export function DateDiffField(props: TableFieldProps) {
  ...
  const date =
    props.torrent[props.fieldName] > 0
      ...
      : "";
  const seconds =
    Math.floor(Date.now() / 1000) - props.torrent[props.fieldName];
  return (
    <div title={date} style={{ width: "100%", textAlign: "right" }}>
      {seconds < 30
        ...
        : date === ""
        ? ""
        : t("time.ago", { time: secondsToHumanReadableStr(seconds) })}
    </div>
  );
}
...
function PercentBarField(props: TableFieldProps) {
  ...
  return (
    <ProgressBar now={now} className="white-outline" {...progressbarStyle} />
  );
}
...
function getRequiredFields(
  ...
  const set = ColumnRequiredFields.reduce((set: Set<TorrentFieldsType>, f) => {
...
export function TorrentTable(props: {
  ...
  const onRowDoubleClick = useCallback(
    (torrent: Torrent, reveal: boolean = false) => {
      if (TAURI) {
        if (torrent.downloadDir === undefined || torrent.downloadDir === "")
          return;
        ...
        invoke("shell_open", { path, reveal }).catch(() => {
          notifications.show({
            title: t("toolbar.notifications.errorOpeningPath"),
            message: path,
            color: "red",
          });
        });
  ...
  const selected = useMemo(
    () => Array.from(serverSelected).map(String),
    [serverSelected]
  );
...
function TorrentContextMenu(props: {
  ...
  const onOpen = useCallback(
    (reveal: boolean) => {
      const torrent = serverData.torrents.find(
        (t) => t.id === serverData.current
      );
  ...
  const torrentAction = useCallback(
    (method: TorrentActionMethodsType, successMessage: string) => {
      mutate(
        {
          method,
    ...
    notifications.show({
      message: t(
        serverSelected.size > 1
          ? "contextMenu.magnetsCopied"
          : "contextMenu.magnetCopied"
      ),
      color: "green",
    });
    ...
    <>
      ...
      >
        <Portal>
          ...
          <Menu.Dropdown miw="10rem">
            <Menu.Item
              onClick={() => {
                torrentAction("queue-move-top", "contextMenu.queueUpdated");
              }}
            ...
            <Menu.Item
              onClick={() => {
                torrentAction("queue-move-up", "contextMenu.queueUpdated");
              }}
            ...
            <Menu.Item
              onClick={() => {
                torrentAction("queue-move-down", "contextMenu.queueUpdated");
              }}
            ...
            <Menu.Item
              onClick={() => {
                torrentAction("queue-move-bottom", "contextMenu.queueUpdated");
              }}
      ...
      >
        <Box miw="14rem">
          ...
          <Menu.Item
            onClick={() => {
              torrentAction("torrent-start-now", "contextMenu.started");
            }}
            onMouseEnter={closeQueueSubmenu}
          ...
          <Menu.Item
            onClick={() => {
              torrentAction("torrent-start", "contextMenu.started");
            }}
            onMouseEnter={closeQueueSubmenu}
          ...
          <Menu.Item
            onClick={() => {
              torrentAction("torrent-stop", "contextMenu.stopped");
            }}
            onMouseEnter={closeQueueSubmenu}
          ...
          <Menu.Item
            onClick={() => {
              torrentAction(
                "torrent-verify",
                "contextMenu.verificationStarted"
              );
            }}
            onMouseEnter={closeQueueSubmenu}
          ...
          <Menu.Item
            onClick={() => {
              torrentAction("torrent-reannounce", "contextMenu.reannounced");
            }}
            onMouseEnter={closeQueueSubmenu}
          ...
          >
            {t(
              serverSelected.size > 1
                ? "contextMenu.copyMagnetLinks"
                : "contextMenu.copyMagnetLink"
            )}
          </Menu.Item>
          <Menu.Item
            ref={queueRef}
          ...
          <Menu.Item
            ...
            onMouseEnter={closeQueueSubmenu}
            icon={
              <Icon.XCircleFill size="1.1rem" color={theme.colors.red[6]} />
            }
            disabled={serverSelected.size === 0}
