...
const AllFields: readonly TableField[] = [
  ...
  {
    name: "size",
    label: "tables.headers.size",
    component: ByteSizeField,
    briefField: true,
  },
  { name: "done", label: "tables.headers.done", component: ByteSizeField },
  {
    name: "percent",
    label: "tables.headers.percent",
...
const useColumns = (
  ...
  return useMemo(
    () =>
      AllFields.filter(
        (field) => field.briefField === true || brief !== true
        ...
        const cell = (cellProps: CellContext<FileDirEntryView, unknown>) => {
          return (
            <field.component
              ...
              row={cellProps.row}
              treeName={brief === true ? "filetreebrief" : "filetree"}
    ...
    [brief, fileTree, nameSortFunc, onCheckboxChange, t]
  );
};
  ...
  return (
    <Box>
      <TextInput
        ...
        rightSection={
          <ActionIcon onClick={onSearchClear} title={t("search.clear")}>
...
export function FileTreeTable(props: FileTreeTableProps) {
  ...
  const columns = useColumns(props.brief, props.fileTree, onCheckboxChange);
