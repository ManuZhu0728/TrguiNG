...
function useTable<TData>(
  ...
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>(
    config.getTableColumnVisibility(tablename)
  );
  const [columnOrder, setColumnOrder] = useState<ColumnOrderState>(
    config.getTableColumnOrder(tablename)
  );
  const [columnSizing, setColumnSizing] = useState<ColumnSizingState>(
    config.getTableColumnSizes(tablename)
  );
  const [sorting, setSorting] = useState<SortingState>(
    config.getTableSortBy(tablename)
  );
...
function useSelectHandler<TData>(
  ...
  const onRowClick = useCallback(
    (event: HIDEvent, index: number, lastIndex: number) => {
      ...
      if (event.shiftKey && event.modKey && lastIndex !== -1) {
        ...
        selectedReducer({
          verb: "toggle",
          ids: [getRowId(rows[index].original)],
        });
...
function useSynchronizedScroll(
  fromRef: React.MutableRefObject<HTMLElement | null>,
  toRef: React.MutableRefObject<HTMLElement | null>
): [
  ...
  const fromRefProxy = useCallback(
    (node: HTMLElement | null) => {
...
export type TableSelectReducer = React.Dispatch<{
  verb: "add" | "set" | "toggle";
  ids: string[];
}>;
...
export function useStandardSelect(): [string[], TableSelectReducer] {
  const [selected, selectedReducer] = useReducer(
    (
      old: string[],
      action: { verb: "add" | "set" | "toggle"; ids: string[] }
    ) => {
...
function HeaderRow<TData>({
  ...
  const onColumnMenuItemClick = useCallback(
...
export function TrguiTable<TData>(props: {
  ...
  const [parentRef, rowHeight, virtualizer] = useTableVirtualizer(
  ...
  const [parentRefProxy, headerRefProxy, syncHeaderScroll] =
    useSynchronizedScroll(parentRef, headerRef);
      ...
      >
        ...
        >
          {virtualizer
            ...
            .filter(
              (virtualRow, virtualIndex) =>
                virtualIndex !== 0 || virtualRow.index % 2 === 0
            )
            .map((virtualRow) => {
              const row = table.getRowModel().rows[virtualRow.index];
              return (
                <MemoizedTableRow<TData>
