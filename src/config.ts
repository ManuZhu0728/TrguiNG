...
export type StatusbarSectionName = (typeof StatusbarSections)[number];
...
export type DetailsSectionsName = (typeof DetailsSections)[number];
...
export const DeleteTorrentDataOptions = [
  "default off",
  "default on",
  "remember selection",
] as const;
export const AddTorrentStartOptions = DeleteTorrentDataOptions;
export const AddTorrentPriorityOptions = [
  "default low",
  "default normal",
  "default high",
  "remember selection",
...
export type AddTorrentPriorityOption =
  (typeof AddTorrentPriorityOptions)[number];
...
export class Config {
  ...
  async read() {
    const merge = (
      await import(/* webpackChunkName: "lodash" */ "lodash-es/merge")
    ).default;
    try {
      ...
      if (overrides.color !== undefined) {
        overrides[this.values.interface.theme ?? "light"].color =
          overrides.color;
        overrides.color = undefined;
      }
      if (overrides.backgroundColor !== undefined) {
        overrides[this.values.interface.theme ?? "light"].backgroundColor =
      ...
      if (this.values.interface.progressbarStyle !== undefined) {
        this.values.interface.animatedProgressbars =
          this.values.interface.progressbarStyle === "animated";
        this.values.interface.colorfulProgressbars =
          this.values.interface.progressbarStyle === "colorful";
    ...
    this.values.servers = this.values.servers.map((s) => ({
      ...s,
      connection: {
        ...s.connection,
        password: deobfuscate(s.connection.password),
      },
    }));
  ...
  async save() {
    ...
    values.servers = values.servers.map((s) => ({
      ...s,
      connection: {
        ...s.connection,
        password: obfuscate(s.connection.password),
      },
    }));
  ...
  getOpenServers(): ServerConfig[] {
    return this.values.servers.filter((s) =>
      this.values.app.openTabs.includes(s.name)
    );
  }
  ...
  async tryMergeInterfaceSettings(obj: Record<string, unknown>) {
    ...
    if (
      ...
      throw new Error(
        "File does not appear to contain valid trguing interface settings"
      );
    }
...
const rotation =
  "HfxgnvteJy86p0ZPDdIYmc5r[kOj9EowACSWG7is%VNBXMhKFa2UlL31bzu4RQTq:";
